...
{
	if (!IsOnlyInstance(m_gameTitle.c_str()))
		return false;

	const DWORDLONG neededDiskSpace = 10LL * 1024LL * 1024LL;
	if (!CheckStorage(neededDiskSpace))
		return false;

	const DWORD minCpuSpeed = 1300; //1.3GHZ
	DWORD curCpuSpeed = ReadCPUSpeed();
	if (curCpuSpeed < minCpuSpeed) {
		std::cerr << "CPU is too slow";
		return false;
	}
...
LRESULT D3DApp::OnClose()
{
	//Close subsystems in reverse order from witch they were created

	/***Delete Game***/
	/***Destroy Window***/
	/***??Destroy Newtwork event forwarder??***/
...
void D3DApp::FlashWhileMinimized() {
	if (!m_hMainWnd) {
		return;
	}

	//If our window (App) is minimized
	if (IsIconic(m_hMainWnd)) {
		DWORD now = timeGetTime();
		DWORD then = now;
		MSG msg;
		FlashWindow(m_hMainWnd, true);
		while (true)
		{
...
bool D3DApp::InitMainWindow()
{
	...
	wc.lpszMenuName  = 0;
	wc.lpszClassName = m_gameTitle.c_str();
	...
	m_hMainWnd = CreateWindow(m_gameTitle.c_str(), m_gameTitle.c_str(),
...
void D3DApp::CalculateFrameStats()
{
	...
	if( (m_Timer.TotalTime() - timeElapsed) >= 1.0f )
	{
        ...
        std::wstring windowText = m_gameTitle +
            L"    fps: " + fpsStr +
            L"   mspf: " + mspfStr;
